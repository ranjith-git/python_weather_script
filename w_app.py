...
for x in contry_info:
    if x["name"].casefold() == c_name.casefold() or x['country'].casefold() == c_name.casefold():
        lat = x["latitude"]
        lon = x["longitude"]
        c_code = x["country"]
        c_name = x['name']
...
def local_time(time):
    ...
    return dt.datetime.strftime(
        dt.datetime.fromtimestamp(time).now(tz), "%b %d, %Y %H:%M:%S %Z"
    )


timezone = read_json["timezone"]
current_time = local_time(read_json["current"]["dt"])
sunrise_time = time_stamp(read_json["current"]["sunrise"])
sunset_time = time_stamp(read_json["current"]["sunset"])

print(f"latitude: {lat},longitude: {lon}, country_code: {c_code}, country_name: {c_name}")
